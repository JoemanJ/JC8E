...
class RAM{
    ...
    public:
        ...
        byte_t read(addr_t address){return memory.at(address);}

        // Writes a single byte of data to address
        void write(addr_t address, byte_t data){memory.at(address) = data;}
...
template <typename T>
void RAM::bulkWrite(addr_t startAddress, addr_t size, const T &data)
{
...
template <typename T>
void RAM::bulkWrite(addr_t startAddress, const T &data){
    const byte_t* source = std::data(data);
